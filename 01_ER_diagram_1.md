...
## 全体
- 具体的な不足テーブルや不足データの抽出をしてください。
- 入荷情報はどのように管理しますか？
- 更新日、作成日のカラムがありません。
- カーディナリティ(0以上か1以上かを表す記号)が不完全なのでもう一度関係性を見直してください。

## 管理者
- 商品との関係性については指摘していませんが、何か理由はありますか？

## 決済
- テーブル名は適していますか？
- 購入日とcreated_atカラムは同じデータが格納されませんか？
- 配送先の郵便番号、配送先の氏名、合計金額、消費税、送料はどのようにして保存しますか？
- 決済とカートの関係性は大丈夫ですか？

## 購入履歴
- テーブル名は適していますか？
- 購入した商品の数量はどのようにして管理しますか？
- PKがありません。
- 発送状況のデータを購入履歴のテーブルに格納することは適していますか？
- 小計(購入時の商品購入金額)のデータの格納するカラムがありません。

## 商品
- 在庫数カラムは入荷数から購入数を引けば算出できるかと思います。
- 商品が削除された場合、該当のレコードどうなりますか？
- 購入履歴を保存する際、商品情報はどのようにして取得しますか？

## カート
- アーティスト、アルバム名、ジャケット画像、合計金額カラムは関連したテーブルのカラムから取得できます。
- この関係性ですと誰のカート情報か識別できません。関係性をもう一度見直してください。
